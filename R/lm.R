...
#      arfactor <- gkernsm(arfactor,rep(hmax,3)*0.42445)$gkernsm
  ...
  if (keep == "all") {
    result <- list(beta = beta, cbeta = cbeta, var = variance, res =
              ...
              data$dim, hrf = z %*% contrast, scorr2=corr2, resscale=scale)
    ...
    result <- list(cbeta = cbeta, var = variance, res = residuals, rxyz = rxyz, scorr =
              ...
              scorr2=corr2, res=NULL, resscale=NULL)
  } else {
    result <- list(cbeta = cbeta, var = variance, rxyz = rxyz, scorr = corr, weights =
              data$weights, vwghts = vwghts, dim = data$dim, hrf = z %*% contrast, res=NULL, resscale=NULL)
  }
...
fmri.lmold <- function(data,z,actype="accalc",hmax=3.52,vtype="var",step=0.01,contrast=c(1),vvector=c(1),keep="essential") {
  cat("fmri.lm: entering function\n")
